#Использовать logos
#Использовать csv
#Использовать "C:\Git\v8metadata-reader"
	...
	Для каждого цПоля Из данныеФайла Цикл
		...
		Если Не _ВыгружатьОшибкиОбъектов
			И (Не ЗначениеЗаполнено(положение)
				ИЛИ Не СтрНачинаетсяС(ВРег(положение), "LINE")) Тогда
...
Процедура ПереопределитьПути(СтрокаТаблицы)
	...
	Если СтрНачинаетсяС(ВРег(СтрокаТаблицы.Положение), "LINE") Тогда
	...
	Если СтрЗаканчиваетсяНа(ВРег(мета), ".FORM") Тогда
		
		// Вешаем на модуль формы
		
		СтрокаТаблицы.Метаданные = мета + ".Module";
		
	ИначеЕсли СтрРазделить(мета, ".").Количество() = 2 Тогда
		
		Если ПутьКМетаданнымСуществует(мета + ".ObjectModule") Тогда
			
			СтрокаТаблицы.Метаданные = мета + ".ObjectModule";
			
		ИначеЕсли ПутьКМетаданнымСуществует(мета + ".ManagerModule") Тогда
			
			СтрокаТаблицы.Метаданные = мета + ".ManagerModule";
			
		ИначеЕсли ПутьКМетаданнымСуществует(мета + ".RecordSetModule") Тогда
			
			СтрокаТаблицы.Метаданные = мета + ".RecordSetModule";
			
		ИначеЕсли ПутьКМетаданнымСуществует(мета + ".ValueManagerModule") Тогда
			
			СтрокаТаблицы.Метаданные = мета + ".ValueManagerModule";
			
		ИначеЕсли ПутьКМетаданнымСуществует(мета + ".CommandModule") Тогда
			
			СтрокаТаблицы.Метаданные = мета + ".CommandModule";
			
		Иначе
			
			СтрокаТаблицы.Метаданные = "Configuration.ManagedApplicationModule";
			СтрокаТаблицы.Описание = мета + ": " + СтрокаТаблицы.Описание;
			
		КонецЕсли;
		
	ИначеЕсли СтрНачинаетсяС(ВРег(мета), "SUBSYSTEMS.") Тогда
	...
	СтрокаТаблицы.Положение = "line 1";
...
Процедура ЗаполнитьВТаблицеРезультатовНомераСтрок(таблицаРезультатов)
	...
	Для каждого цСтрока Из таблицаРезультатов Цикл
		...
		цСтрока.НомерСтроки = СтрЗаменить(ВРег(цСтрока.Положение), "LINE ", "");
